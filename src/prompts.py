...
- build_star_index(genome_id, star_params=None, force_rebuild=False, results_dir=None)：缺少索引时构建；支持完整STAR索引参数优化(sjdbOverhang、genomeSAindexNbases等)；优先传入 results_dir=fastp_results.results_dir 以将参数文件写入 {results_dir}/params。
...
- scan_genome_files()：检查 genomes.json 配置与文件状态（可选，用于判断是否下载/索引）。
- download_genome_assets(genome_id, force=False)：缺少 FASTA/GTF 时下载。
- build_hisat2_index(genome_id, hisat2_params=None, force_rebuild=False, results_dir=None)：缺少索引时构建；支持完整HISAT2索引参数优化(large_index、offrate、ss、exon等)；优先传入 results_dir=fastp_results.results_dir 以将参数文件写入 {results_dir}/params。
